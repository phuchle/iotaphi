...
function show_serviceTracking($events, $user)
{
	...
	if (count($events) > 0)	//to make it null if no person is selected.
		$total['d'] = 0;
	array_unshift($events,array('date' => '', 'event_name' => 'Starting service requirement', 'h' => -22, 'ppl' => 0, 'mi' => 0, 'event_id' => 0)); // add a dummy event to the beginning of events array for baseline hours requirement
