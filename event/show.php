...
if($event_id) {
	...
	//Added 5/11/15 for D4K Katrina
	if($class=='PNO')
		$modifiable = true;

	//ADDEd 5/11/15 for Service VP to be able to modify
	if($class=='service')
		$modifiable = true;
...
function show_eventDescription($event, $shifts, $modifiable, $class, $id)
{
    ...
    <tr><th>Name</th><td colspan="2"><?= $event["name"] ?></td>
    	...
    	//echo "Is there a linebreak? = ";
    	$break_or_not = is_numeric($linebreak);
    	//echo $break_or_not;
    	//echo "<br> linebreak = "; echo $linebreak;
    	$search_location1 = substr($location, 0, $linebreak);
    	$search_location2 = substr($location, $linebreak+7, $length);
		...
		//end of code revision 9/15/2016
