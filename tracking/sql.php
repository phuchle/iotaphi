...
function getEvents($type, $ic)
{
	...
	if($type == '*' && $ic)
	{
		$sql = " SELECT event_id, event_name, UNIX_TIMESTAMP(event_date) AS date FROM event "
			. " WHERE event_date < FROM_UNIXTIME($date) AND event_date > " . db_currentClass('start') . " AND event.eventtype_id NOT IN (1,7) "
			. ' AND event_ic > 0 ORDER BY event_date';
	}
	else
	{
		$sql = " SELECT event_id, event_name, UNIX_TIMESTAMP(event_date) AS date FROM event "
			. " WHERE event_date < FROM_UNIXTIME($date) AND event_date > " . db_currentClass('start') . " AND event.eventtype_id = $type";
		if($ic == false)
			$sql .= ' AND event_ic = 0 ';
