...
if(isset($_POST['search'])):
	...
	foreach($terms as $term)
	{
		$search .= " AND (user_name LIKE '%$term%' OR "
				. " user_address LIKE '%$term%' OR user_snapchat LIKE '%$term%' OR "
...
if(isset($_SESSION['sort']))
{
	switch($_SESSION['sort'])
	{
		...
		case 'aim': $sort = 'user_snapchat'; break;
...
function userfull_get($search = false, $sort = false)
{
	...
	. ' LEFT(user_email, 27) AS user_email, user_snapchat, user_id, class_nick '
				...
				dataOut.sort( compare );
					...
					for ( var i in dataOut )
					{
						...
						for ( var j in dataOut[ i ] )
						{
							...
							} else if ( j=="user_snapchat" ) {
								proc += "<td><img class=\"aimstatus\" src=\"/images/snapchat.jpg" + dataOut[ i ][ j ] + "\" /><a href=\"aim:goim?screenname=" + dataOut[ i ][ j ] + "\">" + dataOut[ i ][ j ] + "</a></td>";
							} else {
		...
		function Search() {
			$.ajax( {
				...
				success: function( data ) {
					...
					var ret = eval( "(" + data + ")" );
					var proc = '<tr><th><div id="sortName">Name</div></th><th><div id="sortAddress">Address</div></th><th><div id="sortCell">Phone</div></th><th><div id="sortText">Text</div></th><th><div id="sortEmail">Email</div></th><th><div id="sortAim">Snapchat <img src="/images/sc_gif.gif" /></div></th><th><div id="sortClass">Class</div></th></tr>';
					...
					for ( var i in ret )
					{
						...
						for ( var j in ret[ i ] )
						{
							...
							} else if ( j=="user_snapchat" ) //SNAP CHAT FOr ROSTER SEARCH 
								{ 

									if(ret[i][j] == 0)
										proc += "<td><img class=\"aimstatus\" src=\"/images/blank.jpg" + "\" /><a href=\"https://snapwebchat.com/login" + "\">" + " " + ret[ i ][ j ] + "</a></td>";

									if(ret[i][j] != 0)
										proc += "<td><img class=\"aimstatus\" src=\"/images/smallsnapchat.jpg" + "\" /><a href=\"https://snapwebchat.com/login" + "\">" + " " + ret[ i ][ j ] + "</a></td>";
